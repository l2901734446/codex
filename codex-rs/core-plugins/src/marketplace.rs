...
fn marketplace_plugin_manifest_fallback(
    ...
    // Claude marketplace entries may contain any plugin manifest field in addition to
    // marketplace-only fields. Preserve those fields in the staged fallback manifest so newly
    // supported Codex manifest fields do not need corresponding translation logic here.
