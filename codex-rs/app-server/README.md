...
- `mcpToolCall` — `{id, server, tool, status, arguments, appContext, mcpAppResourceUri?, pluginId, result?, error?}` describing MCP calls; `appContext` is `{id, linkId}` for calls through a trusted MCP app, where `id` correlates with `app/list` and `linkId` identifies the app link. `tool` identifies the invoked action and `mcpAppResourceUri` points to the widget template. `status` is `inProgress`, `completed`, or `failed`.
